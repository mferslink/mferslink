...
import BaseInfoCard from "@/components/Collection/BaseInfoCard";
...
//     const res = await getCollectionById(ctx.query.id);
//     return {
//         props: {
//             result: res
//         }
...
const CollectionDetail = () => {
    ...
    const collectionIdRef: MutableRefObject<string> = useRef('');
    ...
    useEffect(
        () => {
            if (isBottom && !isLoading && !isEmpty) {
              ...
              getCollectionComments(curSortType);
            }
        },
        [curSortType, isBottom, isLoading, isEmpty]
    );

    useEffect(() => {
        collectionIdRef.current = router.query.id as string;
        userIdRef.current = userInfo.walletAddress;
    }, [router.query.id, userInfo]);

    const fetchCommentTotal = useCallback(async () => {
        const total = await getCommentNums(collectionIdRef.current, 'collection');
        setCommentsTotal(total);
    }, [collectionIdRef.current]);

    const getCollectionComments = useCallback((sortType, useCdn = false) => {
        setIsLoading(true);
        if (collectionIdRef.current) {
            getComments(collectionIdRef.current, 'collection', sortType, curPage, 10, useCdn).then((res) => {
                if (res.length === 0) {
                    setIsEmpty(true);
                    setIsLoading(false);
                    return;
    ...
    }, [isPosting, curSortType, curPage, isLoading]);

    useEffect(() => {
        getCollectionById(collectionIdRef.current, true).then(result => {
            setBaseInfo(result || {});
        });
        getCollectionComments(curSortType, true);
        fetchCommentTotal();
    ...
    const onPostComment = useCallback(async (params) => {
        ...
        try {
            ...
            await postComment(Object.assign(params, {
                ...
                commentToID: collectionIdRef.current,
                commentToUploaderID: baseInfo.uploaderID,
            }));
            setIsPosting(false);
            setCurPage(0);
            getCollectionComments(SortType.New);
            fetchCommentTotal();
    ...
    return <div className={styled.wrap}>
        ...
        <div>
            ...
            />
            {
                ...
                && comments.length > 0
                && <CommentList
                    typeId={collectionIdRef.current}
                    list={comments}
